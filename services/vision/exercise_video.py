...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        options = vision.PoseLandmarkerOptions(
            ...
            min_pose_detection_confidence=0.7,   #detector  and connection confidence values are same.
        ...
        for start_idx, end_idx in POSE_CONNECTIONS:
            ...
            if p1.visibility > 0.7 and p2.visibility > 0.7:
                cv2.line(
                    ...
                    (int(p2.x * w), int(p2.y * h)),   # w = width of image and h = height of image,.
                    (0, 255, 0),
                    8
                )
        
        for lm in landmarks:   #circle for visible landmarks.
    ...
    def _draw_no_pose_warnings(self, img):   # IF NO POSE OR IMAGE DETECTED THEN SHOW THIS WARNING MESSAGE. USING OPENCV 
        cv2.putText(   # PUTTEXT PUT TEXT OVER IMAGE.
            img,
            "NO POSE DETECTED",
            (30, 50),
        ...
        # OVERLAYS function of each class.
    ...
    def recv(self, frame):
        image = np.asarray(
            cv2.flip(frame.to_ndarray(format="bgr24"), 1),  # flip image bcz mirror shows flip image
            dtype=np.uint8
        )

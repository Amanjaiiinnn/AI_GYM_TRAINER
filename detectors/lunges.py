...
class LungesDetector(BaseExercise):  #assign threshold values for angles and visibility, and indices for key landmarks
    DOWN_THRESHOLD = 100
    UP_THRESHOLD = 160
    MIN_VISIBILITY = 0.7
    ...
    def process(self, landmarks) -> dict:    
        ...
        if left_knee_angle <= right_knee_angle:   # check which knee is in front .
        ...
        if lateral_offset <= self.BALANCE_TOLERANCE:    #torso balnce status for lunge detection and feedback based on horizontal distance between shoulder midpoint and hip midpoint.

...
class PushUpDetector(BaseExercise):
    ...
    def process(self, landmarks) -> dict:
        ...
        shoulder_y = landmarks[shoulder_idx].y   # calculate shoulder y, ankle y and hip y for hip sagging or piked up status for pushup detection and feedback.
        ankle_y = landmarks[ankle_idx].y
        hip_y = landmarks[hip_idx].y

        expected_hip_y = (shoulder_y + ankle_y) / 2
        hip_deviation = hip_y - expected_hip_y       # hip deviation from expected position to determine sagging or piked up status.

...
#MORE OR LESS CODE IS SAME AS LANDMARKS DETECTION AND ANGLE LOGIC IS MOSTLY SAME.
...
class BicepsCurlDetector(BaseExercise):
    ...
    def process(self, landmarks) -> dict:
        ...
        elbow_drift = abs(elbow_x - shoulder_x)   # shows elbow drift from shoulderin verticle line.
        ...
        dx = shoulder_mid_x - hip_mid_x # calculate horizontal and vertical distance between shoulder midpoint and hip midpoint to determine torso angle and swinging status for biceps curl detection and feedback.
    ...
    def _safe_angle(self, dx, dy):
        return math.degrees(math.atan2(abs(dx), abs(dy))) if dy != 0 else 0.0  #HERE instead of acose ve use atan.
